...
latest_mainnet_release=$(curl -s "https://api.github.com/repos/Mystenlabs/sui/releases?per_page=10" | jq '.' | jq -r '.[] | select(.tag_name | startswith("mainnet")) | .tag_name' | head -n 1);
